...
export default function LeaderboardScreen({ profile, onBack, theme, embedded }) {
    ...
    <div style={{ minHeight: embedded ? 'auto' : '100vh', background: embedded ? 'transparent' : t.bg, color: t.text, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'center' }}>
        <div style={{ width: '100%', maxWidth: 720, padding: '36px 40px', animation: 'fadeUp 0.4s ease' }}>
          ...
          <div style={{ fontSize: 11, color: '#f1be43', fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', marginBottom: 6 }}>SACE Chemistry</div>
                  ...
                  {[board[1], board[0], board[2]].map((p, pos) => {
                    ...
                    return (
                      <div key={p.id} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
                        ...
                        <div style={{ width: 76, height: heights[pos], background: isMe ? 'rgba(241,190,67,0.12)' : t.bgCard, border: `2px solid ${isMe ? '#f1be43' : colors[pos]}`, borderRadius: '12px 12px 6px 6px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 4, boxShadow: theme === 'light' ? '0 2px 8px rgba(0,0,0,0.08)' : 'none' }}>
                          <div style={{ fontSize: 11, fontWeight: 800, color: isMe ? '#f1be43' : colors[pos], textAlign: 'center', padding: '0 6px' }}>{p.display_name.split(' ')[0]}</div>
                ...
                {board.map((p, i) => {
                  ...
                  return (
                    <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 18px', background: isMe ? 'rgba(241,190,67,0.08)' : t.bgCard, border: `1px solid ${isMe ? 'rgba(241,190,67,0.4)' : t.border}`, borderRadius: 12, boxShadow: theme === 'light' ? '0 1px 3px rgba(0,0,0,0.05)' : 'none' }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 14, fontWeight: 700, color: isMe ? '#f1be43' : t.text }}>{p.display_name}{isMe ? ' (You)' : ''}</div>
                        {p.school && <div style={{ fontSize: 11, color: t.textMuted, marginTop: 1 }}>{p.school}</div>}
                      </div>
                      <div style={{ textAlign: 'right' }}>
                        <div style={{ fontSize: 14, fontWeight: 800, color: '#f1be43' }}>{p.xp.toLocaleString()} XP</div>

        ...
        <div style={{ padding: '14px 16px', borderBottom: `1px solid ${t.border}` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 11, color: t.accent, fontWeight: 600 }}>{RANK_ICONS[Math.min(getLevel(profile.xp), RANK_ICONS.length-1)]} {RANKS[Math.min(getLevel(profile.xp), RANKS.length-1)]}</div>
            </div>
          </div>
        </div>
        <div style={{ flex: 1 }} />
        ...
        <div style={{ maxWidth: 700 }}>
          ...
          {loading ? (
            ...
            <>
                  ...
                  {[board[1], board[0], board[2]].map((p, pos) => {
                    ...
                    return (
                      <div key={p.id} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
                        ...
                        <div style={{
                          width: 76, height: heights[pos],
                          background: isMe
                            ? theme === 'dark' ? 'rgba(20,184,166,0.15)' : 'rgba(13,148,136,0.1)'
                            : t.bgCard,
                          border: `2px solid ${isMe ? t.accent : colors[pos]}`,
                          borderRadius: '12px 12px 6px 6px',
                          display: 'flex', flexDirection: 'column',
                          alignItems: 'center', justifyContent: 'center', gap: 4,
                          boxShadow: theme === 'light' ? '0 2px 8px rgba(0,0,0,0.08)' : 'none',
                        }}>
                          <div style={{ fontSize: 11, fontWeight: 800, color: isMe ? t.accent : colors[pos], textAlign: 'center', padding: '0 6px' }}>
                            {p.display_name.split(' ')[0]}
                          </div>
                          <div style={{ fontSize: 10, color: t.textMuted, fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: 600 }}>
                            {p.xp.toLocaleString()}
                          </div>
                        </div>
                      </div>
                ...
                {board.map((p, i) => {
                  ...
                  return (
                    <div key={p.id} style={{
                      ...
                      padding: '14px 18px',
                      background: isMe
                        ? theme === 'dark' ? 'rgba(20,184,166,0.08)' : 'rgba(13,148,136,0.06)'
                        : t.bgCard,
                      border: `1px solid ${isMe ? t.accent + '66' : t.border}`,
                      borderRadius: 12,
                      boxShadow: theme === 'light' ? '0 1px 3px rgba(0,0,0,0.05)' : 'none',
                      transition: 'background 0.15s',
                    }}>
                      <div style={{ fontSize: 14, fontWeight: 800, width: 28, textAlign: 'center', color: i < 3 ? [t.xp, t.textSub, '#cd7c2f'][i] : t.textFaint, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
                        #{i + 1}
                      </div>
                      ...
                      <div style={{ textAlign: 'right' }}>
                        <div style={{ fontSize: 14, fontWeight: 800, color: t.xp, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
                          {p.xp.toLocaleString()} XP
                        </div>
                        {p.streak > 0 && (
                          <div style={{ fontSize: 11, color: t.danger, marginTop: 2 }}>🔥 {p.streak} streak</div>
                        )}
                      </div>
                    </div>
                  )
              ...
              {myPosition > 0 && (
                <div style={{ marginTop: 16, padding: '12px 18px', background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: 10, fontSize: 13, color: t.textMuted, textAlign: 'center', boxShadow: theme === 'light' ? '0 1px 3px rgba(0,0,0,0.05)' : 'none' }}>

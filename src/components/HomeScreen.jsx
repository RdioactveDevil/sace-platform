...
export default function HomeScreen({ profile, struggleMap, questions, subject, onStartSession, theme }) {
  ...
  const toggleExpandedTopic = (topic) => {
  ...
  const toggleTopic = (topic) => {
    ...
    if (allMode) {
      const allSubs = questions.map(q => q.subtopic).filter((v, i, a) => a.indexOf(v) === i)
  ...
  return (
    <div style={{ color: t.text, fontFamily: FONT_B, animation: 'hs-fadeUp 0.3s ease', flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <style>{`
        ...
        .hs-topic-row { display: grid; grid-template-columns: auto auto minmax(0, max-content) minmax(120px, 1fr) auto; align-items: center; gap: 10px; padding: 10px 16px; border-bottom: 1px solid ${t.border}; }
        .hs-topic-arrow { width: 18px; height: 18px; display: inline-flex; align-items: center; justify-content: center; background: transparent; border: none; color: ${t.textMuted}; font-size: 12px; cursor: pointer; padding: 0; }
        .hs-topic-progress { height: 4px; border-radius: 999px; overflow: hidden; background: ${t.border}; }
        .hs-topic-progress-fill { height: 100%; background: ${GOLD}; border-radius: 999px; }
        @media (max-width: 860px) {
          ...
          .hs-topic-row { grid-template-columns: auto auto minmax(0, 1fr) auto; row-gap: 6px; }
          .hs-topic-progress { grid-column: 3 / 5; }
        }
      `}</style>
              ...
              <div style={{ ...card, padding: 0, overflow: 'hidden' }}>
                ...
                <div style={{ maxHeight: 340, overflowY: 'auto' }}>
                  {Object.entries(topicGroups).map(([topic, tg]) => {
                    ...
                    const pct = tg.total > 0 ? Math.round((tg.attempted / tg.total) * 100) : 0
                    const isExpanded = !!expandedTopics[topic]

                    return (
                      <div key={topic}>
                        <div className="hs-topic-row">
                          <div
                            onClick={() => toggleTopic(topic)}
                            style={{ width: 16, height: 16, borderRadius: 4, border: `2px solid ${topicSel || topicPartial ? GOLD : t.border}`, background: topicSel ? GOLD : 'transparent', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}
                          >
                          ...
                          <button
                            type="button"
                            className="hs-topic-arrow"
                            onClick={() => toggleExpandedTopic(topic)}
                          ...
                          <span style={{ fontSize: 13, fontWeight: 700, color: t.text, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                            {topic} ({tg.attempted}/{tg.total})
                          </span>

                          <div className="hs-topic-progress">
                            <div className="hs-topic-progress-fill" style={{ width: `${pct}%` }} />
                          </div>

                          <span style={{ fontSize: 12, color: t.textMuted, fontWeight: 700, textAlign: 'right', minWidth: 44 }}>{pct}%</span>
                        </div>
                        ...
                        {isExpanded && subs.map(sub => {
                          ...
                          return (
                            <div key={sub} onClick={() => toggleSub(sub)}
                              style={{ display: 'flex', alignItems: 'center', padding: '7px 16px 7px 50px', cursor: 'pointer', borderBottom: `1px solid ${t.border}`, background: subSel ? 'rgba(241,190,67,0.04)' : 'transparent' }}>

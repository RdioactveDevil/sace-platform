            ...
            <>
                  ...
                  {[board[1], board[0], board[2]].map((p, pos) => {
                    ...
                    return (
                      <div key={p.id} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
                        ...
                        <div style={{ width: 76, height: heights[pos], background: isMe ? (theme==='dark'?'rgba(241,190,67,0.15)':'rgba(241,190,67,0.1)') : t.bgCard, border: `2px solid ${isMe ? t.accent : colors[pos]}`, borderRadius: '12px 12px 6px 6px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 4, boxShadow: theme==='light'?'0 2px 8px rgba(0,0,0,0.08)':'none' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {board.map((p, i) => {
                  ...
                  return (
                    <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 18px', background: isMe ? (theme==='dark'?'rgba(241,190,67,0.08)':'rgba(241,190,67,0.06)') : t.bgCard, border: `1px solid ${isMe ? t.accent+'66' : t.border}`, borderRadius: 12, boxShadow: theme==='light'?'0 1px 3px rgba(0,0,0,0.05)':'none' }}>
